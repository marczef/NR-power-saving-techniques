function [energy_gain, delays] = wus_drx_script(T_total, longDRX_cycle, inactivity_timer, ...
    on_duration, frequency_of_incoming_packet, packet_reception_time, ps_Offset, minTimeGap)
    % Input parameters
    ...
    fprintf('ps-Offset time: %d ms\n', ps_Offset);
    fprintf('minTimeGap time: %d ms\n\n', minTimeGap);
        ...
        switch current_state
            ...
            case 'on_duration'
                ...
                elseif time_until_packet >= 0 && time_until_packet <= on_duration
                    ...
                    EE_DRX = E_DRX + 100 * (time_to_process - packet_reception_time) + 300 * packet_reception_time;
            ...
            case 'wus_signal'
                if time_until_packet < 0
                    % Packet arrived in previous cycle in sleep
                    fprintf('WAKE UP SIGNAL : 1\n[Packet #%d arrived during SLEEP at last cycle at %d ms]\n', next_packet_index, current_time + time_until_packet);
                                        
                    wake_up_indication = 1;

                    previous_state = 'wake_up_signal';
                    current_state = 'sleep';
                    fprintf('-> State change: WAKE_UP SIGNAL -> ON DURATION (Cycle #%d)\n', cycle_count);
                else
                    % Any packet did not arrive - wake up_indication - 0
                    fprintf('WAKE UP SIGNAL : 0 - no packet arrived \n');
                                        
                    wake_up_indication = 0;

                    previous_state = 'wake_up_signal';
                    current_state = 'sleep';
                    fprintf('-> State change: WAKE_UP SIGNAL -> SLEEP (Cycle #%d)\n', cycle_count);
                end
                
                time_to_process = min((ps_Offset - minTimeGap), remaining_time);
                E_DRX = E_DRX + 100 * time_to_process;
                remaining_time = remaining_time - time_to_process;
                fprintf('[Energy after WUS] Total: %.1f mJ | Saved: %.1f%%\n', E_DRX, current_gain);
                
            case 'sleep'
                % Calculate remaining sleep time

                if strcmp(previous_state, 'wake_up_signal') && wake_up_indication == 1 % szybki sen na 1 lub 3 sloty
                    sleep_time_remaining = minTimeGap;
                    previous_state = 'sleep';
                    current_state = 'on_duration';
                    wake_up_indication = 0;

                elseif strcmp(previous_state, 'wake_up_signal') && wake_up_indication == 0 % długi sen z pominięciem on_duration
                    sleep_time_remaining = longDRX_cycle - (ps_Offset - minTimeGap);
                    previous_state = 'sleep';
                    current_state = 'wus_signal';
                    cycle_count = cycle_count + 1;

                elseif strcmp(previous_state, 'inactivity_timer')
                    sleep_time_remaining = longDRX_cycle - on_time_counter - ps_Offset;
                    previous_state = 'sleep';
                    current_state = 'wus_signal';
                    cycle_count = cycle_count + 1;
                end
