function [energy_gain, delays] = c_drx_script(T_total, longDRX_cycle, inactivity_timer, ...
    on_duration, frequency_of_incoming_packet, packet_reception_time, shortDRX_cycle, shortCycleTimer)

    % Input parameters
    fprintf('\n=== Starting C-DRX Simulation ===\n');
    fprintf('Total simulation time: %d ms\n', T_total);
    fprintf('Long DRX cycle: %d ms\n', longDRX_cycle);
    fprintf('Short DRX cycle: %d ms\n', shortDRX_cycle);
    fprintf('Short DRX cycle timer: %d\n', shortCycleTimer);
    ...
    delays_array = [];
    timer_short_cycle = shortCycleTimer;
    short_DRX_counter = 0;
    ...
    if shortCycleTimer ~= 0
        is_short_drx_active = true;
        drx_cycle = shortDRX_cycle;
    else 
        is_short_drx_active = false;
        drx_cycle = longDRX_cycle;
    end
        ...
        switch current_state
            ...
            case 'on_duration'
                if time_until_packet < 0
                    ...
                    % C-DRX update
                    if is_short_drx_active == false && shortCycleTimer ~= 0
                        is_short_drx_active = true;
                        drx_cycle = shortDRX_cycle;
                        timer_short_cycle = shortCycleTimer;
                    end
            ...
            case 'sleep'
                ...
                sleep_time_remaining = drx_cycle - on_time_counter;

                if sleep_time_remaining <= 0
                    sleep_time_remaining = drx_cycle;
                end
                ...
                if is_short_drx_active == true
                    timer_short_cycle = timer_short_cycle - 1;
                    short_DRX_counter = short_DRX_counter + shortDRX_cycle;
                    fprintf('short_DRX_counter: %d\n', short_DRX_counter);
                    fprintf('Short DRX Timer short cycle zostało: %d\n', timer_short_cycle);
                end

                if timer_short_cycle == 0 && is_short_drx_active == true
                    is_short_drx_active = false;
                    on_time_counter = mod(short_DRX_counter, longDRX_cycle);
                    fprintf('short_DRX_counter: %d\n', short_DRX_counter);
                    fprintf('reszta po shortDRXie: %d\n', on_time_counter);
                    drx_cycle = longDRX_cycle;
                    short_DRX_counter = 0;
                end
